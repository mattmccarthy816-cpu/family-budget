...
const FAMILY_MEMBERS = ["Mom", "Dad", "Alex", "Jordan"];
...
const MEMBER_COLORS = {
  "Mom": "#f472b6", "Dad": "#60a5fa", "Alex": "#fbbf24", "Jordan": "#4ade80",
};
...
function StatusIcon({ status }) {
  ...
  return (
    <span style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:18, height:18, borderRadius:"50%", background:s.color+"20", color:s.color, fontSize:11, fontWeight:800, fontFamily:"'DM Mono', monospace", flexShrink:0 }}>{s.icon}</span>
  );
}
...
function BudgetBar({ spent, budget, dayOfMonth, daysInMonth }) {
  ...
  return (
    <div style={{ background:"#1e293b", borderRadius:999, height:7, position:"relative" }}>
...
function Modal({ onClose, children }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div style={{ background:"#0f172a", border:"1px solid #1e293b", borderRadius:20, padding:28, width:"100%", maxWidth:400, maxHeight:"90vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const entries = (data.entries || [])
        .filter(r => r[0])
        .map(r => ({ id:String(r[0]), date:String(r[1]), member:String(r[2]), category:String(r[3]), amount:parseFloat(r[4]) }));
      ...
      const maxId = entries.reduce((m, e) => Math.max(m, parseInt(e.id)||0), 0);
      setNextId(maxId + 1);
    } catch(err) {
      setError("Couldn't connect to Google Sheets. " + err.message);
  ...
  const isLatestMonth = isCurrentMonth;
  ...
  const byMember = useMemo(() => {
    const map={}; FAMILY_MEMBERS.forEach(m=>map[m]=0);
    entries.forEach(e=>{map[e.member]=(map[e.member]||0)+e.amount;}); return map;
  }, [entries]);

  const byCategory = useMemo(() => {
    const map={}; categories.forEach(c=>map[c]=0);
    entries.forEach(e=>{if(map[e.category]!==undefined) map[e.category]+=e.amount;}); return map;
  }, [entries, categories]);

  const byMemberCategory = useMemo(() => {
    const map={};
    FAMILY_MEMBERS.forEach(m=>{map[m]={};categories.forEach(c=>{map[m][c]=0;});});
    entries.forEach(e=>{if(map[e.member]&&map[e.member][e.category]!==undefined) map[e.member][e.category]+=e.amount;}); return map;
  }, [entries, categories]);

  const categoryStatuses = useMemo(() => {
    const map={}; categories.forEach(c=>{map[c]=getCategoryStatus(byCategory[c]||0,budgets[c],dayOfMonth,daysInMonth);}); return map;
  }, [byCategory, budgets, dayOfMonth, daysInMonth, categories]);
  ...
  const sortedCategories = useMemo(() => [...categories].sort((a,b)=>{
    const order={over:0,risk:1,warn:2,ok:3};
    return order[categoryStatuses[a]]-order[categoryStatuses[b]];
  ...
  async function handleAddSubmit() {
    if (!form.member||!form.category||!form.amount||isNaN(+form.amount)||+form.amount<=0) {
      showToast("Please fill all fields.", false); return;
    }
    ...
    setSyncing(true);
    try {
      await api({ action:"addEntry", ...entry });
      setAllEntries(prev=>[entry,...prev]);
      setNextId(n=>n+1);
      setForm({member:"",category:"",amount:""});
      showToast(`${fmtD(entry.amount)} logged for ${entry.member}.`);
      setView("dashboard");
    } catch { showToast("Failed to save. Check your connection.", false); }
  ...
  async function saveEditEntry() {
    if (!editForm.member||!editForm.category||!editForm.amount||isNaN(+editForm.amount)||+editForm.amount<=0) {
      showToast("Please fill all fields.", false); return;
    }
    const updated = {...editEntry,...editForm,amount:parseFloat((+editForm.amount).toFixed(2))};
    setSyncing(true);
    try {
      await api({ action:"updateEntry", ...updated });
  ...
  async function deleteEntry(id) {
    ...
    try {
      ...
      setEditEntry(null); showToast("Entry deleted.");
    } catch { showToast("Failed to delete.", false); }
    finally { setSyncing(false); }
  }

  async function saveBudgetsToSheet(newBudgets, newColors) {
    const payload = Object.keys(newBudgets).map(c=>({category:c,budget:newBudgets[c],color:newColors[c]||PALETTE[0]}));
    try { await api({ action:"saveBudgets", budgets:JSON.stringify(payload) }); }
  ...
  async function saveCat() {
    ...
    let newBudgets={...budgets}, newColors={...catColors};
    if (editCat==="new") {
      if (budgets[name]) { showToast("Already exists.",false); return; }
      newBudgets[name]=parseFloat((+catForm.budget).toFixed(2)); newColors[name]=catForm.color;
    } else {
      if (name!==editCat&&budgets[name]) { showToast("Name taken.",false); return; }
      if (name!==editCat) { newBudgets[name]=newBudgets[editCat]; delete newBudgets[editCat]; newColors[name]=newColors[editCat]; delete newColors[editCat]; setAllEntries(prev=>prev.map(e=>e.category===editCat?{...e,category:name}:e)); }
      newBudgets[name]=parseFloat((+catForm.budget).toFixed(2)); newColors[name]=catForm.color;
    }
    setBudgets(newBudgets); setCatColors(newColors);
    setEditCat(null); showToast(editCat==="new"?`"${name}" added.`:"Category updated.");
    await saveBudgetsToSheet(newBudgets, newColors);
  }
  async function deleteCat(catName) {
    if (categories.length<=1) { showToast("Can't delete the last category.",false); return; }
    const newBudgets={...budgets}; delete newBudgets[catName];
    const newColors={...catColors}; delete newColors[catName];
    setBudgets(newBudgets); setCatColors(newColors);
    setAllEntries(prev=>prev.filter(e=>e.category!==catName));
    setEditCat(null); showToast(`"${catName}" deleted.`);
    await saveBudgetsToSheet(newBudgets, newColors);
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#020817", fontFamily:"'Sora','DM Mono',sans-serif", color:"#f1f5f9", paddingBottom:48 }}>
      ...
      {/* Header */}
      <div style={{ background:"#0a0f1e", borderBottom:"1px solid #1e293b", padding:"0 20px" }}>
        <div style={{ maxWidth:700, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:64 }}>
          <div>
            <div style={{ fontSize:18, fontWeight:700, letterSpacing:-0.5, display:"flex", alignItems:"center", gap:8 }}>
              ...
              {alertCount>0 && (
                <span style={{ background:overCount>0?"#ef444422":"#f9731622", color:overCount>0?"#ef4444":"#f97316", border:`1px solid ${overCount>0?"#7f1d1d":"#7c2d12"}`, fontSize:10, fontWeight:800, borderRadius:999, padding:"2px 8px", fontFamily:"'DM Mono',monospace" }}>
                  {alertCount}
                </span>
              )}
              {syncing && <span style={{ fontSize:10, color:"#475569", fontFamily:"'DM Mono',monospace" }}>syncing…</span>}
            </div>
            <div style={{ fontSize:11, color:"#334155", fontFamily:"'DM Mono',monospace", marginTop:1 }}>
              {MONTH_NAMES[viewMonth].toUpperCase()} {viewYear} · DAY {dayOfMonth}/{daysInMonth}
            </div>
          </div>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position:"fixed", top:20, left:"50%", transform:"translateX(-50%)", background:toast.ok?"#052e16":"#450a0a", border:`1px solid ${toast.ok?"#16a34a":"#dc2626"}`, color:toast.ok?"#4ade80":"#f87171", padding:"11px 22px", borderRadius:12, fontSize:13, fontWeight:600, zIndex:500, boxShadow:"0 8px 32px rgba(0,0,0,0.5)", animation:"slideUp 0.3s ease", maxWidth:320, textAlign:"center" }}>
          {toast.msg}
        </div>
      )}
      ...
      {editEntry && (
        <Modal onClose={()=>setEditEntry(null)}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div>
              <div style={{ fontSize:11, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:7 }}>Member</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                {FAMILY_MEMBERS.map(m=>(
                  <button key={m} className="chip" onClick={()=>setEditForm(f=>({...f,member:m}))} style={{ background:editForm.member===m?MEMBER_COLORS[m]+"25":"#0a0f1e", border:`1px solid ${editForm.member===m?MEMBER_COLORS[m]:"#1e293b"}`, color:editForm.member===m?MEMBER_COLORS[m]:"#475569" }}>{m}</button>
                ))}
              </div>
            </div>
            <div>
              <div style={{ fontSize:11, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:7 }}>Category</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                {categories.map(c=>(
                  <button key={c} className="chip" onClick={()=>setEditForm(f=>({...f,category:c}))} style={{ background:editForm.category===c?catColors[c]+"20":"#0a0f1e", border:`1px solid ${editForm.category===c?catColors[c]:"#1e293b"}`, color:editForm.category===c?catColors[c]:"#475569" }}>{c}</button>
                ))}
              </div>
            </div>
            <div>
              <div style={{ fontSize:11, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:7 }}>Amount</div>
              <div style={{ position:"relative" }}>
                <span style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", color:"#475569" }}>$</span>
                <input className="input-field" type="number" min="0" step="0.01" value={editForm.amount} onChange={e=>setEditForm(f=>({...f,amount:e.target.value}))} style={{ paddingLeft:28 }} />
              </div>
            </div>
            <div>
              <div style={{ fontSize:11, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:7 }}>Date</div>
              <input className="input-field" type="date" value={editForm.date} onChange={e=>setEditForm(f=>({...f,date:e.target.value}))} />
            </div>
            <div style={{ display:"flex", gap:10, marginTop:6 }}>
              <button className="submit-btn" onClick={saveEditEntry} disabled={syncing} style={{ flex:1 }}>Save Changes</button>
              <button className="danger-btn" onClick={()=>deleteEntry(editEntry.id)} disabled={syncing}>Delete</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {editCat!==null && (
        <Modal onClose={()=>setEditCat(null)}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div>
              <div style={{ fontSize:11, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:7 }}>Name</div>
              <input className="input-field" type="text" placeholder="e.g. Subscriptions" value={catForm.name} onChange={e=>setCatForm(f=>({...f,name:e.target.value}))} />
            </div>
            <div>
              <div style={{ fontSize:11, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:7 }}>Monthly Budget</div>
              <div style={{ position:"relative" }}>
                <span style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", color:"#475569" }}>$</span>
                <input className="input-field" type="number" min="0" placeholder="0" value={catForm.budget} onChange={e=>setCatForm(f=>({...f,budget:e.target.value}))} style={{ paddingLeft:28 }} />
              </div>
            </div>
            <div>
              <div style={{ fontSize:11, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:10 }}>Color</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {PALETTE.map(p=>(
                  <div key={p} className="color-swatch" style={{ background:p, borderColor:catForm.color===p?"#fff":"transparent", boxShadow:catForm.color===p?"0 0 0 1px #fff":"none" }} onClick={()=>setCatForm(f=>({...f,color:p}))} />
                ))}
              </div>
            </div>
            <div style={{ display:"flex", gap:10, marginTop:6 }}>
              <button className="submit-btn" onClick={saveCat} style={{ flex:1 }}>{editCat==="new"?"Add Category":"Save Changes"}</button>
              {editCat!=="new" && <button className="danger-btn" onClick={()=>deleteCat(editCat)}>Delete</button>}
            </div>
          </div>
        </Modal>
      )}
            ...
            {view==="dashboard" && (
              <div className="fade-in">
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
                  ...
                  <button className="month-btn" onClick={nextMonth} disabled={isLatestMonth}>
                    {isLatestMonth ? "—" : `${MONTH_NAMES[viewMonth===11?0:viewMonth+1].slice(0,3)} →`}
                  </button>
                </div>
                ...
                <div style={{ background:"#0f172a", borderRadius:16, padding:24, marginBottom:16, border:"1px solid #1e293b" }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
                    {sortedCategories.map(c=>{
                      ...
                      const projected=Math.round((byCategory[c]||0)/(dayOfMonth/daysInMonth));
                      return (
                        <div key={c}>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6, gap:8 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                              <div style={{ width:8, height:8, borderRadius:2, background:catColors[c]||"#94a3b8", flexShrink:0 }} />
                              <span style={{ fontSize:13, fontWeight:600, color:"#e2e8f0" }}>{c}</span>
                              <StatusIcon status={status} />
                            </div>
                            <div>
                              <span style={{ fontSize:13, fontWeight:700, color:iconColor, fontFamily:"'DM Mono',monospace" }}>{fmt(byCategory[c]||0)}</span>
                              <span style={{ fontSize:12, color:"#334155", fontFamily:"'DM Mono',monospace" }}> / {fmt(budgets[c])}</span>
                            </div>
                          </div>
                          <BudgetBar spent={byCategory[c]||0} budget={budgets[c]} dayOfMonth={dayOfMonth} daysInMonth={daysInMonth} />
                          {status!=="ok" && isCurrentMonth && (
                            <div style={{ fontSize:11, color:iconColor, marginTop:4, fontFamily:"'DM Mono',monospace", opacity:0.8 }}>
                              {status==="over" ? `${fmt((byCategory[c]||0)-budgets[c])} over limit` : `Projected EOM: ${fmt(projected)} · ${fmt(projected-budgets[c])} over`}
                            </div>
                          )}
                        </div>
                      );
                      ...
                      <div key={m}>
                        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            <div style={{ width:8, height:8, borderRadius:"50%", background:MEMBER_COLORS[m] }} />
                ...
                <div style={{ background:"#0f172a", borderRadius:16, padding:24, marginBottom:16, border:"1px solid #1e293b" }}>
                  ...
                  <div style={{ overflowX:"auto" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                      <thead>
                        <tr>
                          <th style={{ textAlign:"left", color:"#475569", padding:"0 0 12px", fontWeight:600 }}>Category</th>
                          {FAMILY_MEMBERS.map(m=>(<th key={m} style={{ textAlign:"right", color:MEMBER_COLORS[m], padding:"0 0 12px 12px", fontWeight:600 }}>{m}</th>))}
                        </tr>
                      </thead>
                      <tbody>
                        {categories.filter(c=>(byCategory[c]||0)>0).map(c=>(
                          <tr key={c} style={{ borderTop:"1px solid #1e293b" }}>
                            <td style={{ padding:"9px 0", color:"#94a3b8", display:"flex", alignItems:"center", gap:6 }}>
                              <span style={{ width:6, height:6, borderRadius:1, background:catColors[c]||"#94a3b8", display:"inline-block" }} />{c}
                            </td>
                            {FAMILY_MEMBERS.map(m=>(
                              <td key={m} style={{ textAlign:"right", padding:"9px 0 9px 12px", color:(byMemberCategory[m]&&byMemberCategory[m][c])>0?"#e2e8f0":"#1e293b", fontFamily:"'DM Mono',monospace" }}>
                                {(byMemberCategory[m]&&byMemberCategory[m][c])>0?fmt(byMemberCategory[m][c]):"—"}
                              </td>
                            ))}
                          </tr>
                        ))}
                ...
                <div style={{ background:"#0f172a", borderRadius:16, padding:24, border:"1px solid #1e293b" }}>
                  <div style={{ fontSize:13, fontWeight:600, color:"#64748b", textTransform:"uppercase", letterSpacing:2, marginBottom:16, fontFamily:"'DM Mono',monospace" }}>
                    Entries <span style={{ color:"#334155", fontSize:10, fontWeight:400 }}>· tap to edit</span>
                  </div>
                  {entries.length===0 ? (
                    <div style={{ textAlign:"center", padding:"32px 0", color:"#334155", fontSize:13 }}>No entries for {MONTH_NAMES[viewMonth]}.</div>
                    ...
                    <div key={e.id} className="entry-row" onClick={()=>openEditEntry(e)}>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        ...
                        <div>
                          <div style={{ fontSize:13, fontWeight:600, color:"#e2e8f0" }}>{e.member}</div>
              ...
              <div className="slide-up" style={{ maxWidth:420, margin:"0 auto" }}>
                <div style={{ background:"#0f172a", borderRadius:20, padding:32, border:"1px solid #1e293b" }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
                    <div>
                      <div style={{ fontSize:12, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:8 }}>Who are you?</div>
                      <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                        {FAMILY_MEMBERS.map(m=>(
                          <button key={m} className="chip" onClick={()=>setForm(f=>({...f,member:m}))} style={{ background:form.member===m?MEMBER_COLORS[m]+"25":"#0a0f1e", border:`1px solid ${form.member===m?MEMBER_COLORS[m]:"#1e293b"}`, color:form.member===m?MEMBER_COLORS[m]:"#475569" }}>{m}</button>
                        ))}
                      </div>
                    </div>
                    <div>
                      ...
                      <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                        {categories.map(c=>{
                          ...
                          return (
                            <button key={c} className="chip" onClick={()=>setForm(f=>({...f,category:c}))} style={{ background:isSelected?catColors[c]+"20":"#0a0f1e", border:`1px solid ${isSelected?catColors[c]:status!=="ok"?STATUS_ICON[status].color+"50":"#1e293b"}`, color:isSelected?catColors[c]:status!=="ok"?STATUS_ICON[status].color:"#475569" }}>
                              {status!=="ok"&&<StatusIcon status={status} />}{c}
                            </button>
                          );
                        })}
                      </div>
                      {form.category && (
                        <div style={{ marginTop:10, padding:"10px 14px", borderRadius:10, background:"#0a0f1e", border:"1px solid #1e293b" }}>
                          <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:6 }}>
                            <span style={{ color:"#64748b" }}>{form.category}</span>
                            <span style={{ color:STATUS_ICON[categoryStatuses[form.category]].color, fontFamily:"'DM Mono',monospace", fontWeight:700 }}>{fmt(byCategory[form.category]||0)} / {fmt(budgets[form.category])}</span>
                          </div>
                          <BudgetBar spent={byCategory[form.category]||0} budget={budgets[form.category]} dayOfMonth={dayOfMonth} daysInMonth={daysInMonth} />
                        </div>
                      )}
                    </div>
                    <div>
                      <div style={{ fontSize:12, color:"#64748b", fontFamily:"'DM Mono',monospace", textTransform:"uppercase", letterSpacing:1.5, marginBottom:8 }}>Amount</div>
                      <div style={{ position:"relative" }}>
                        <span style={{ position:"absolute", left:16, top:"50%", transform:"translateY(-50%)", color:"#475569", fontSize:16 }}>$</span>
                        <input className="input-field" type="number" min="0" step="0.01" placeholder="0.00" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} style={{ paddingLeft:32 }} />
                      </div>
                    </div>
              ...
              <div className="fade-in" style={{ maxWidth:460, margin:"0 auto" }}>
                <div style={{ background:"#0f172a", borderRadius:20, padding:28, border:"1px solid #1e293b" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                    <div style={{ fontSize:20, fontWeight:700 }}>Categories</div>
                    <button onClick={openNewCat} style={{ background:"#1e40af", border:"none", borderRadius:9, color:"#fff", fontSize:12, fontWeight:700, padding:"7px 14px", cursor:"pointer", fontFamily:"'Sora',sans-serif" }}>+ New</button>
                  </div>
                  ...
                  <div style={{ display:"flex", flexDirection:"column" }}>
                    {categories.map(c=>{
                      ...
                      const iconColor=STATUS_ICON[status].color;
                      return (
                        <div key={c} onClick={()=>openEditCat(c)} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"13px 10px", borderBottom:"1px solid #0f172a", gap:8, cursor:"pointer", borderRadius:8, transition:"background 0.15s" }}
                          onMouseEnter={e=>e.currentTarget.style.background="#0f172a88"}
                          onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                            <div style={{ width:12, height:12, borderRadius:3, background:catColors[c]||"#94a3b8", flexShrink:0 }} />
                            <span style={{ fontSize:14, color:"#e2e8f0", fontWeight:600 }}>{c}</span>
                            <StatusIcon status={status} />
                          </div>
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            <span style={{ fontSize:13, fontWeight:700, color:iconColor, fontFamily:"'DM Mono',monospace" }}>{fmt(byCategory[c]||0)}</span>
                  ...
                  <div style={{ marginTop:20, padding:"14px 16px", background:"#0a0f1e", borderRadius:12, border:"1px solid #1e293b", display:"flex", justifyContent:"space-between" }}>
                    <span style={{ fontSize:13, color:"#64748b", fontWeight:600 }}>Total budget</span>
                    <span style={{ fontSize:14, fontWeight:700, color:"#f1f5f9", fontFamily:"'DM Mono',monospace" }}>{fmt(totalBudget)} / month</span>
                  </div>
                </div>
              </div>
            )}
